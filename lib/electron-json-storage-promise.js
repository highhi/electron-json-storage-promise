...
function getFilePath(storageKey) {
  ...
  const escapedKey = encodeURIComponent(
    `${path.basename(key, '.json')}.json`
  );
...
function set(key, data) {
  ...
  return fs.writeJSON(getFilePath(key), data)
